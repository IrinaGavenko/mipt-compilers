...
namespace tableNS {
    ...
    public:
        ...
        void AddSuperClassName(const std::string &symbol) { 
		superClass = StringSymbol::GetIntern(symbol); 
	}

        void AddSuperClassName(const StringSymbol *symbol) {
		superClass = symbol; 
	}

        const std::vector<const StringSymbol *> &GetMethodNames() const { 
		return methodsName; 
	}

        const std::vector<const StringSymbol *> &GetVarsNames() const {
		return varsName; 
	}
        ...
        const StringSymbol *GetSuperClassName() const {
		return superClass; 
	}
